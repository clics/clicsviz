...
var linkByWords = {};
d3.json('user/words.json', function(words){
...
var langByInfo = {};
var coords = [];
d3.json('user/langsGeo.json', function(langs){
...
console.log("langbyinfo", langByInfo);
...
d3.json(filename, function(data){
  ...
  console.log('nodes', nodes)
	...
	links.push({
	  ...
	  edge_width : 0.25 * data.adjacency[i][j].weight // addon JML
	});
      }
    };
  ...
  .style('cursor','pointer')
	...
	.html(function(){
	  ...
	  for (_i=0; _i<wofam.length; _i++) {
	    spl = wofam[_i].split('/');
      console.log("testing", spl[3]);
      if (spl[3] in langByInfo){
	      infolist.push([
          spl[4], 
          linkByWords[spl[0]], 
          spl[0], '?'+spl[0], 
